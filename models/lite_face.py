...
class LiteFace(nn.Module):
    def __init__(self, in_channels: int, image_size: tuple, c1: int = 64, c2: int = 64, c3: int = 64, c4: int = 64, f2: int = 32):
        ...
        avg_pool_ = ("avg_pool", 7, 1, 0)
        adaptive_avg_pool_ = ("adaptive_avg_pool", (7, 7))
        self.conv_layers = (
            ("conv", c1, 3, 2, 3),
            max_pool_,
            ("conv", c2, 3, 1, 1),
...
if __name__ == '__main__':
    in_channels, image_height, image_width = 3, 250, 250
    model = LiteFace(in_channels, (image_height, image_width))
    print(model.name)
    print(model)
    model.eval()
    if 1:
        torch_input = torch.randn(1, in_channels, image_height, image_width)
        torch.onnx.export(model, torch_input, "lite_face.onnx")
