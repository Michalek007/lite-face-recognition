...
class ShuffleFaceNet(nn.Module):
    def __init__(self, in_channels: int, image_size: tuple, c1=64, c2=64, c3=64, c4=64, f2=32):
        super().__init__()
        ...
        adaptive_avg_pool_ = ("adaptive_avg_pool", (5, 5))
        self.conv_layers = (
            ("conv", c1, 3, 2, 3),
            max_pool_,
            ("conv", c2, 3, 1, 1),
...
if __name__ == '__main__':
    ...
    model = ShuffleFaceNet(in_channels, (image_height, image_width), 32, 8)
    print(model.name)
    print(model)
    model.eval()
    if 1:
        torch_input = torch.randn(1, in_channels, image_height, image_width)
        torch.onnx.export(model, torch_input, "shuffle_face_net.onnx")
